...
{	
	...
	while (1)
	{
		...
		if (shm_ipc_addr->mgr_printf[2] == 2)
		{
			LOG_DEBUG(s_i4LogMsgId,\
				"\n *****************************************************************************\n"
				"\r LockFlag:0x%x   ProhibitTranFlag:0x%x     HalfRssi_high:%u   HalfRssi_low:%u  rssi_to_db:%d\n"
				"\r F1S1:0x%X     F1S2:0x%X     F2S1:0x%X     F2S2:0x%X\n"
				"\r HalfFd:%u     HalfFd:%u     HalfFd:%u     HalfFd:%u\n"
				"\r *****************************************************************************\n",\
				fpga_mem_nm_debug->lock_flag,fpga_mem_nm_debug->prohibit_transmit_flag,((fpga_mem_nm_debug->u_half_rssi)>>16)&0xffff,(fpga_mem_nm_debug->u_half_rssi)&0xffff,rssi_convert_db(((fpga_mem_nm_debug->u_half_rssi)>>16)&0xffff,(fpga_mem_nm_debug->u_half_rssi)&0xffff),\
...
void find_db_table(unsigned int rssi,unsigned int rssi_base,unsigned int db_base,float factor,int *convert_db )
{
    float rssi_range[10]={0};
    int i=0;
    float  base=rssi_base;
    ...
    *convert_db=0;
    ...
    for(i=0;i<10;i++)
    {
        rssi_range[i]=rssi_base*db_convert[i]*factor;
    }
    for(i=0;i<10;i++)
    {
        if((rssi>=base)&&(rssi<=rssi_range[i]))
        {
            *convert_db=db_base-(i+1);
            break;
        }
        else
        {
    ...
    if(10==i)
    {
        printf("find_db_table error\n");
    }
    
}

int rssi_convert_db(unsigned int high,unsigned int low)
{
    int convert_db;
    if(high>=600)
    {
    ...
    {
        find_db_table(high,200,60,0.3,&convert_db);
    }
    else if(high > 20)
    {
        find_db_table(high,20,70,1,&convert_db);
    }
    else if(high == 20)
    {
        convert_db=70;
    ...
    {
        find_db_table(high,2,80,1,&convert_db);
    }
    else if(high == 2)
    {
        convert_db=80;
    ...
    {
        find_db_table(low,3000,100,1,&convert_db);
    }
    else if(low == 3000)
    {
        convert_db=100;
    ...
    {
        find_db_table(low,300,110,1,&convert_db);
    }
    else if(low == 300)
    {
        convert_db=110;
    ...
    {
        find_db_table(low,30,120,1,&convert_db);
    }
    else if(low == 30)
    {
        convert_db=120;
    ...
    {
        find_db_table(low,3,130,1,&convert_db);
    }
    else  
    {
        convert_db=130;
