 ...
 *   *************************************************************************/
 /**
 * @brief   ½«Ô´ID ºÍÄ¿µÄID ÒÔÊ®½øÖÆ ´òÓ¡³öÀ´£¬·½±ãµ÷ÊÔ¿´
 * @author ÖÜ´óÔª
 * @since
 * @bug
 */
void PrintIDbyDec(unsigned char *src, unsigned char *dst)
{
    ...
    {
        LOG_DEBUG(s_LogMsgId,"[CCL] \tsnd=%d rcv=%d\n", src[0], dst[0]);
    }
    else
    {
...
void  IDP_CclPrintDllData(unsigned char *DllData)
{
    ...
    LOG_DEBUG(s_LogMsgId,"[CCL][%s]MsgType=%d FrmType=%d DataType=%#x ValidLen=%d ", __FUNCTION__,
                                                                             ptDllData->MsgType,
                                                                             ptDllData->FrmType,
                                                                             ptDllData->DataType,
                                                                             ptDllData->DataLen);
