...
static int  pfDllSet(UINT32 u4Arc, CHAR* pcArg[])
{
    if(0 ==u4Arc)
    {
		...
		CLI_Printf("set  ccerrmax [0-6]                  - Set dll cc err max superframe count .\n");
    }
    ...
    else if(0 == strncmp(pcArg[0], "ccerrmax", sizeof("ccerrmax")))
    {
		if(2 == u4Arc)
		{
...
static int  pfMGRGet(UINT32 u4Arc, CHAR* pcArg[])
{
     ...
     if(0 ==u4Arc)
    {
        ...
        CLI_Printf("get web_info    - Get Web Info.\n");
    }
     else if(1 ==u4Arc) 
	{
        ...
        else if(0 == strncmp(pcArg[0], "web_info", sizeof("web_info")))
		{
			CLI_Printf("Web user: %s    code:%s\n", ptCFGShm->web_user.buf,ptCFGShm->web_code.buf);
		}
		else
		{
			CLI_Printf("Bad command input! type \"set\" for help!\n");
