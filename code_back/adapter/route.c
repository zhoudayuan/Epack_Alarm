 ...
 * @bug
 */
void Route_AI_do_Send(INT32 socketfd,UINT8 *pInData,UINT32 u4RouteLen,ROUTE_AI_RECV_TYPE_E u4RouteType)
{
	if((ROUTE_AI_RECV_TYPE_CONFIG_SERVER <= u4RouteType)||(0 > socketfd))
		return;
 ...
 * @bug
 */
void Route_AI_Send(INT32 socketfd,UINT8 *pInData,UINT32 u4RouteLen,ROUTE_AI_RECV_TYPE_E u4RouteType)
{
	if((ROUTE_AI_RECV_TYPE_MAX <= u4RouteType)||(0 > socketfd))
		return;
...
#else
		ptRoutePrintf->uUpBusy = 0;
		u4RouteLen = read(s_i4AIFd,auRouteBuff,ROUTE_DATA_LEN_MAX);//阻塞方式
		ptRoutePrintf->uUpBusy = 1;
...
static void *Route_Send_Process(void *pvArg)
{
	...
	while(1)
	{
		...
		if(ptTemp)
		{
			...
			write(s_i4AIFd,ptTemp->auData,ROUTE_DATA_LEN_MAX);
			DBC_Free(ptTemp);	
		}
...
{	
	...
	if((s_i4AIFd = open("/dev/axiram-service",O_RDWR)) < 0)
	{
	...
	fcntl(s_i4AIFd, F_SETOWN, getpid());
	fcntl(s_i4AIFd, F_SETFL, fcntl(s_i4AIFd, F_GETFL) | FASYNC);
