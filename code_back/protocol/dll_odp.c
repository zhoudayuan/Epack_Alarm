 ...
 *     DEI数据结束指示。结束特征图样用来指示有效数据结束。
 *         00 最后帧数据全空，倒数第二帧以特征图样结束
 *         01 最后帧数据未满，以特征图样结束
 *         10 倒数第二帧数据全满， 且数据在倒数第二帧结束，最后帧全填0
 *         11 最后帧数据全满
 *     SAP用来指示数据分组的业务类型，例如IP业务其值为4。
 *     DPF用来区分有无确认传输方式。
 *     FSN片序号，用于片重组。一共4bit, 最低3bit用于片序号，最高bit用于指示是否为最终片.
 *     12 16 18是什么意思 ?
 */
...
int ODP_NasNeighborFun(CCL_DLL_DL_T * pvCclData)
{
    ...
    if (g_DllGlobalCfg.auNodeId == pvCclData->DstId[0])  // 邻点查询本地
    {
        ptCclData->MsgType = DI_MSG_WLU;
        ptCclData->FrmType = FT_VOICE_NO;
