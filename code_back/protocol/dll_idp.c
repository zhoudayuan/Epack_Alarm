...
int IDP_NasNeighborFun(NAS_AI_PAYLOAD * pvNasData)
{
    ...
    if ((OP_CODE_GET == pvNasData->op_code) && (g_DllGlobalCfg.auNodeId == pvNasData->dst_id))  // 获取
    {
        ...
        LOG_DEBUG(s_LogMsgId, "[DLL][%s][%d] g_NerbValidFlg=%d, g_BurstCnt=%lu, g_BurstInTalkingCnt=%d", _F_, __LINE__, g_NerbValidFlg, g_BurstCnt, g_BurstInTalkingCnt);

        if (g_NerbValidFlg == 0)
        {
            memset(&ptCclData->PayLoad[1], 0xff, NER_LEN);
        }
        else if (g_NerbValidFlg == 1)
        {
    ...
    else if (OP_CODE_GET_ACK == pvNasData->op_code && g_DllGlobalCfg.auNodeId == pvNasData->dst_id)  // 响应
    {
...
void INFVoiceProcess(NAS_INF_UL_T * pvInfData, UINT8 RevFrqNo)
{
    ...
    else if (g_DllGlobalCfg.auMSCC != pDataLink->CC)
    {
        ...
        if(!CC_RIGHT_FLG || (tDllPrint->Ccerrsuperfrmcnt <= CCERR_SUPERFRM_COUNT))
        {
            LOG_WARNING(s_LogMsgId,"[DLL][%s] voice CC: %d error abandon:  ERR_COUNT:%d SUPERFRM_COUNT:%d CC_RIGHT_FLG=%d",_F_, pDataLink->CC,
                                                                                              CC_ERR_COUNT,CCERR_SUPERFRM_COUNT,CC_RIGHT_FLG);
            return;
        }
        else
        {
            ...
            LOG_WARNING(s_LogMsgId,"[DLL][%s] voice CC: %d error  ERR_COUNT:%d SUPERFRM_COUNT:%d",_F_, pDataLink->CC,CC_ERR_COUNT,CCERR_SUPERFRM_COUNT);
        }
    }
    else
    {
    ...
    switch (pDataLink->FrmType)
    {
        case FT_VOICE_A:
        {
            DLL_SetTimer(CALL_ERR_VOICE_U, 1500);  // 定时器初始化
...
//    IDP_PrintInfDataLog(pDataLink);  // by zhoudayuan
    switch (pDataLink->DataType)
    {
        ...
        case DT_LC_HEADER:
        {
            if (ptErrPrint->Voice == 1)
            {
                if (g_ErrVoiceFlag == ERR_VOICE_START)
                {
                    test_err_voice_init();
                    g_ErrVoiceFlag = ERR_VOICE_OVER;
                }
            }
            g_BurstCnt = 0;
            CC_RIGHT_FLG=0;
