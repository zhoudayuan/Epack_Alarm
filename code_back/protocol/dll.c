...
/**
 * @var g_BurstInTalkingCnt
 * @brief 在通话过程中，该计数器有效, 记录突发周期的个数
 */
UINT8 g_BurstInTalkingCnt = 0;
        ...
        switch(uCallState)
        {
            ...
            case CALL_ERR_VOICE_U:
            {
                ...
                if (uWaitErrTimes > uCallWait)
                {
                    ...
                    {
                        /*
                        **  如果test_err_voice_result() 被调用过，则g_ErrVoiceFlag 设置1
                        **  该标志用来区分test_err_voice_result 是在超时调用还是DT_LC_TERMINATOR 调用
                        */
                        if (g_ErrVoiceFlag == ERR_VOICE_OVER)
                        {
                            test_err_voice_result(); // 误码率-语音测试-结果
                            g_ErrVoiceFlag = ERR_VOICE_START;
                        }
                    }
                    DLL_ClearTimer();
                }
...
void * DLL_NerBurstTask(void * p)
{
    ...
    */
    ...
    while(1)
    {
        ...
        val = rand() % BurstCyc;  // 取值单位周期内的某个时间点
        tv.tv_sec = val / 1000;  // 秒级赋值
        tv.tv_usec = val % 1000 * 1000; // 毫秒级赋值
        LeftDelay = BurstCyc / 1000 - tv.tv_sec;  // 单位周期，突发后的剩余时间，毫秒忽略不计


        ret = select(0, NULL, NULL, NULL, &tv);  // 仅延时
        if (ret < 0)
        {
            printf("[DLL] TIM  select error\n");
            continue;
        ...
        /*
        ** 邻点突发, 突发本节点ID号和邻点信息(链路机(0~31)中哪个是邻点)
        **
        */
        ...
        sleep(LeftDelay);  // 完整的邻点突发周期

        g_NbrCycCnt++;
        /*
        ** 当语音通话时，在其他线程某处 g_BurstCnt=0, 启动另一个计时器 g_BurstInTalkingCnt。
        ** g_BurstInTalkingCnt 记录 [2个邻点突发周期]
        ** g_NerbValidFlg = 1 当语音呼叫开始随后的两个邻点周期之内，及语音呼叫结束后的两个邻点周期之后
        ** g_NerbValidFlg = 0 当语音呼叫开始随后的两个邻点周期之后 直到 语音呼叫结束后的两个邻点周期之内
        */
       /* if (g_BurstCnt == 0||g_BurstCnt == 1)  // 只有当该链路机处于通话状态时，该标志g_BurstCnt 始终为 0
        {
            g_BurstInTalkingCnt++;
            if( g_NbrCycCnt%2 == 0)// 语音通话处在偶数周期
            {
                if(1==g_BurstInTalkingCnt)
                {
                    g_BurstInTalkingCnt = 0;
                    g_NerbValidFlg = 0;  // 设置邻点查询功能不可用。
                }
                printf("[%s:%d] WLU NbrCycCnt:%d BurstCnt:%d\n", __FUNCTION__, __LINE__,g_NbrCycCnt,g_BurstCnt);
            }
            else
            {
                if (g_BurstInTalkingCnt == 2)
                {
                    g_BurstInTalkingCnt = 0;
                    g_NerbValidFlg = 0;  // 设置邻点查询功能不可用。
                    printf("[%s:%d] WLU R Talking\n", __FUNCTION__, __LINE__);
                }
                printf("[%s:%d] WLU NbrCycCnt:%d BurstCnt:%d talkcnt:%d\n", __FUNCTION__, __LINE__,g_NbrCycCnt,g_BurstCnt,g_BurstInTalkingCnt);
            }
        }
        else
        {
        ...
        if( (g_NbrCycCnt%2 == 0) &&(g_BurstCnt ==0))
        {
            g_NerbValidFlg = 0;  // 设置邻点查询功能不可用。
            printf("[%s:%d] 1WLU NbrCycCnt:%lu BurstCnt:%lu\n", __FUNCTION__, __LINE__,g_NbrCycCnt,g_BurstCnt);

        }
        else if((g_NbrCycCnt%2 == 0) &&(g_BurstCnt ==1))
        {
            g_NerbValidFlg = 0;  // 设置邻点查询功能不可用。
            printf("[%s:%d] 2WLU NbrCycCnt:%lu BurstCnt:%lu\n", __FUNCTION__, __LINE__,g_NbrCycCnt,g_BurstCnt);
        }
        else
        {
            printf("[%s:%d] 3WLU NbrCycCnt:%lu BurstCnt:%lu\n", __FUNCTION__, __LINE__,g_NbrCycCnt,g_BurstCnt);
        }

        /*
        ** 每两个邻点突发周期，备份一次邻点信息(唯一备份的地方),
        ** 邻点信息的备份(auNegrId2),32bit, 每个bit记录的是[相应邻点是否存在]，其bit索引即为链路机ID号
        ** 用途: 1-断链告警; 2-邻点突发; 3-邻点主动上报；3-邻点查询
        */
        if ((g_BurstCnt % 2 == 0) && (g_BurstCnt != 0))
        {
            // 备份本地邻点信息
            ...
            g_NerbValidFlg = 1;  // 设置邻点查询有效
            // 清除本地邻点信息
            g_DllGlobalCfg.auNegrId1 = 0;
            memset(g_DllGlobalCfg.auNerInfo1, 0, sizeof(g_DllGlobalCfg.auNerInfo1));
            printf("[%s:%d] bak Ner info\n", __FUNCTION__, __LINE__);
        }

        /*
        ** 邻点主动上报，每两个突发周期上报一次
        */
        if (((g_BurstCnt % 2 == 0) && (g_BurstCnt != 0)) && (ptCFGShm->neighbor_report_ai.val == 1))
        {
