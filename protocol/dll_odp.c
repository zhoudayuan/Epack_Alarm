 ...
 * @bug
 */
void CallingShootData(UINT8 MsgT, UINT8 FrmT, UINT8 DataT, UINT8 Len, UINT8 *PayLoad)
{
    UINT8 Frq = (tDllPrint->FrqSlt >> 4) & 0x0f;
    UINT8 slt = tDllPrint->FrqSlt & 0x0f;
...
int ODP_LcHeaderFun(CCL_DLL_DL_T * pvCclData)
{
    ...
    memset(ptInfData, 0, sizeof(NAS_INF_DL_T));
    CallingShootData(DI_MSG_DATA,FT_VOICE_NO,DT_LC_HEADER,(FLC_LEN+3),g_DllGlobalCfg.auDownloadLC);
...
int ODP_GenPreCSBKFun(UINT8 cbf, UINT8* Dst, UINT8* Src, UINT8 GI)
{
    ...
    CallingShootData(DI_MSG_DATA,FT_VOICE_NO,DT_CSBK,(CSBK_LEN+2),(UINT8 *)&tPreCsbk);
...
int ODP_R12UPacketDataFun(CCL_DLL_DL_T * pvCclData)
{
    ...
    //下行DataHeader组包
    memset(ptInfData, 0, sizeof(NAS_INF_DL_T));
    CallingShootData(DI_MSG_DATA,FT_VOICE_NO,DT_DATA_HEADER,(DATA_HEADER_LEN+2),g_PdpDBuf.auHead[0]);
    ODP_SendInfData(ptInfData, S_U_HDR);

    //下行R1/2 Data组包
    for (i = 0; i < Bf; i++)
    {
        CallingShootData(DI_MSG_DATA,FT_VOICE_NO,DT_R_1_2_DATA,R12_NC_DATA_DLEN,pDPData->tPdpR12NcBlk.auData[i]);
...
void CCLVoiceProcess(CCL_DLL_DL_T * pvCclData)
{
    ...
    memset(ptInfData, 0, sizeof(NAS_INF_DL_T));
    CallingShootData(DI_MSG_VOICE,pvCclData->FrmType,pvCclData->DataType,VOICE_LEN,pvCclData->PayLoad);
