...
void savccidnetdata(unsigned char *pvcenterdata,unsigned char *CCcvNetid)
{
	...
	if(1 == tcclPrint->CclDown)
	{
	LOG_DEBUG(s_LogMsgId,"[CCL][%s] CallId=%d,VoiceId=%d,SubNet=%d sendnum :%d %d %d  recnum :%d %d %d", __FUNCTION__,
											                                                  ptCVNetid->CallId,
											                                                  ptCVNetid->VoiceId,
											                                                  ptCVNetid->SubNet,
											                                                  ptCVNetid->srcid[0],
...
{   
    ...
    LOG_DEBUG(s_LogMsgId,"[CCL][%s] SIGTYPE:%d DLEN:%d  ", __FUNCTION__, pvPttcmd->SharedHead.SigType,
																	pvPttcmd->SharedHead.Datalength);
																							    
	if( SIG_PTT_CMD == sigtype)
	{
	    LOG_DEBUG(s_LogMsgId,"[CCL][%s] pttState:%d calingNum[0]:%d calingNum[1]:%d calingNum[2]:%d, CalledNum[0]=%d \
	    																				CalledNum[0]=%d CalledNum[0]=%d",
	...
	{
	    LOG_DEBUG(s_LogMsgId,"[CCL][%s]  calingNum[0]:%d calingNum[1]:%d calingNum[2]:%d CalledNum[0]=%d,\
	    																	CalledNum[0]=%d CalledNum[0]=%d",
			  													 							 __FUNCTION__, 
	...
	{
	    LOG_DEBUG(s_LogMsgId,"[CCL][%s] SmsType:%d validlen=%d SNum[0]:%d SNum[1]:%d SNum[2]:%d,  RNum[0]=%d RNum[1]=%d RNum[2]=%d",
																			  								__FUNCTION__, 
...
void  ODP_CclPrintDllData(unsigned char *DllData)
{
	...
	/*LOG_DEBUG(s_LogMsgId,"[CCL][%s]MsgType=%d  FrmType=%d  DataType=%d ValidLen=%d ", __FUNCTION__,
																			 ...
																			 ptDllData->DataType,
																			 ptDllData->DataLen);*/
	LOG_DEBUG(s_LogMsgId,"[CCL][%s]MSGTYPE=%d, DataType=%d, DataLen:%d SrcId[0]=%d  SrcId[1]=%d SrcId[2]=%d\n",
																				__FUNCTION__,
					                                                     ptDllData->MsgType,
					                                                     ptDllData->DataType,
					                                                     ptDllData->DataLen,
					                                                     ...
					                                                     ptDllData->SrcId[1],
					                                                     ptDllData->SrcId[2]);
	LOG_DEBUG(s_LogMsgId,"[CCL][%s] DstId[0]=%d  DstId[1]=%d DstId[2]=%d\n",__FUNCTION__,
							                                             ptDllData->DstId[0],
							                                             ptDllData->DstId[1],
							                                             ptDllData->DstId[2]);
