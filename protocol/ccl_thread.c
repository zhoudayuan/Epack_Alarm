...
void *ODP_HandleCenterVioce(void *arg)
{
    ...
    {       
		...
		if( u4Datalen  < ( CENTER_VOICE_DL_HEADLEN+CENTER_VOICE_DL_PAYLOADLEN))			//接收数据长度小于公共头长度													  //接收数据长度小于公共头长度
		{
...
{        
	...
	{	
		...
		{
			ODP_CclPrintCcdlsig((unsigned char *)CcCmd);
		}
...
void IDP_MsgVoiceHandle(unsigned char * pvDllData,  unsigned char * CenterData)
{
    ...
    { 
		...
		{
			LOG_DEBUG(s_LogMsgId,"[CCL][%s]SEND CC PTT ON,Rec_LC_FLG=%d   Send_PTTON_FLG=%d ", __FUNCTION__,
																					s_tRec_LC_FLG,
		                                                                            s_tSend_PTTON_FLG); 
		}
...
void IDP_MsgWluHandle(unsigned char * pvDllData,unsigned char *CenterData)
{
	...
	case CT_KILL_ACK_NAS:
		if( Wait_Kill_Nas_Ack == INF_CCL_STATE)
		{
...
{       
	...
	{  
        ...
        case Wait_Reviv_NAS_Ack:
        	...
        	{   
				LOG_DEBUG(s_LogMsgId,"[CCL][%s]NAS SERVICE timeout  ccl_work_state=%d timecount=%d", __FUNCTION__,INF_CCL_STATE,TimeCount);
            	INF_CCL_STATE=INF_CCL_IDLE;
            	TimeCount=0;
        	}
        	break;
